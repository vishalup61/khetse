...
app.use((req,res,next)=>{ const key=req.ip+":"+req.path; const now=Date.now(); const b=rateBuckets.get(key); if(!b||now-b.start>60000){rateBuckets.set(key,{start:now,count:1});return next();} b.count++; if(b.count>120)return res.status(429).json({success:false,message:"Too many requests. Please try again shortly."}); next(); });
app.use(express.static(path.join(__dirname, "..")));

const ADMIN_USERNAME = process.env.ADMIN_USERNAME;
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD;
const ADMIN_TOKEN = process.env.ADMIN_TOKEN;
if(!ADMIN_USERNAME || !ADMIN_PASSWORD || !ADMIN_TOKEN){ console.warn("⚠️ Set ADMIN_USERNAME, ADMIN_PASSWORD and ADMIN_TOKEN in production environment variables."); }
...
app.get("/api/health",(req,res)=>res.json({success:true,message:"Purez API running",database:!!db,environment:process.env.NODE_ENV||"development"}));
...
app.post("/api/orders",async(req,res)=>{
  try{
    ...
    if(incoming.paymentMethod!=="Cash on Delivery")return res.status(400).json({success:false,message:"Online payment abhi available nahi hai. Cash on Delivery select karein."});
...
async function start(){
  try{
    ...
    app.listen(PORT,()=>console.log(`🌾 Purez server running at http://localhost:${PORT}`));
